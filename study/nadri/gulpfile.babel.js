...
// pug와 같이 html 동작할 수 있도록 만들어주는 extender
...
//sass 컴파일을 위한 기본 sass  dart-sass 는 최신 업데이트를 적용중이나 output style 중 compact 미지원

import nodeSass from "node-sass";
// output style compact 지원으로 수정
...
// css의 공백을 없애 로딩을 빠르게 만드는 플러그인
import sourceMap from "gulp-sourcemaps";
// css 위치 확인을 위한 sourcmap 추가 프러그인
...
const routes = {
    ...
    font: {
        watch: "src/scss/fonts/*.{ttf,woff2,woff,eot}",
        src: "src/scss/fonts/*.{ttf,woff2,woff,eot}",
...
const styles = () =>
    gulp
        .src(routes.scss.src)
        .pipe(sourceMap.init()) //sass sourcemap 선언
        .pipe(sass.sync({outputStyle: 'compact'}).on("error", sass.logError))
        .pipe(autoprefixer()) //익스플로러 호환 등 자동으로 prefix추가해주는 플러그인
        // .pipe(miniCSS()) //공백 및 줄바꿈 제거
        .pipe(sourceMap.write('maps')) //sass sourcemap 결과파일 경로
        .pipe(gulp.dest(routes.scss.dest));
//scss의 위치확인 > 컴파일 방식 : 하나의 엘리먼트 당 줄바꿈 없이 저장(compact) >  에러가 있을 경우 알려주는 파이프 > 결과물 저장 위치 경로 지정
...
const js = () => gulp.src(routes.js.src)
    // .pipe(bro({
        // transform: [
            // babelify
                // .configure({presets: ['@babel/preset-env']})
...
const watch = () => {
    ...
    gulp.watch(routes.font.watch, fonts);
