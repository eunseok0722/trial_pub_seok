...
$(function ($) {
  ...
  $('.guide_tabs a').click(function () {
    ...
    $('.' + targetTabsCon).addClass('on'); //페이지갯수 갱신

    tabPerTotalPageNum = $('.tabs_cont.on tbody tr').length;
    setTotalPageNum(tabPerTotalPageNum);
  }); //가이드 리스트 그리기

  drawGuideList(infoGuideList, infoGuideFolder, "guide_list_wrap"); //레이아웃 리스트 그리기

  drawGuideList(infoLayoutList, infoGuideFolder, "layout_list_wrap"); //가이드 리스트 그리기

  drawGuideList(infoPcardList, infoPcardFolder, "page_list_wrap"); // to top
  ...
  $(function () {
    var locN = $('tbody tr').length; // console.log(locN)
